...
while(my $line = <$IN>){
	...
	my $cmd = "samtools sort -n $segs[0] | samtools fastq - | bwa mem -t 8 -p -M -R '\@RG\\tID:$segs[-2]\\tSM:$segs[-1]\\tLB:$segs[-2]' $fasta - | samtools sort -m 2G -o $uname.sorted.bam -T $uname -";
