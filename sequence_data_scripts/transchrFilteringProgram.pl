#!/usr/bin/perl
# This script is designed to process transchr files to find highly frequent events that likely indicate repeats rather than transchr
...
my $usage = "perl $0 -d <input file transchr bedpe> -f <filter bed file> -o <output bed file>\n";
my %opts;
...
open(IN, "< $opts{f}") || die "Could not open filter bed file!\n";
while(my $line = <IN>){
	chomp $line;
	my @segs = split(/\t/, $line);
	if($segs[3] < 3){
	...
	push(@{$beds->chr($segs[0])->bin()->{$bin}}, simpbed->new('start' => $segs[1], 'end' => $segs[2]));
}
